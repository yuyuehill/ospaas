...
class  TestClinder(test_os_base.TestOpenStackBase): 
    ...
    def list_volumes(self):
        params = json.dumps({})        
        dd = self.call_cinder_api('GET', '/volumes', params)
    ...
    def create_volume(self, name, size):
        ...
        dd = self.call_cinder_api("POST", "/volumes", params)       
    ...
    def delete_volume(self, tenant_id, volume_id):
        ...
        dd = self.call_cinder_api("DELETE", "/volumes/%s" % volume_id , params)
        #the delete api will not return any body
        return {"volume_id":volume_id}
    
    def get_volume(self, volume_id):
        dd = self.call_cinder_api("GET", "/volumes/%s" % volume_id, json.dumps({}))
    ...
    def test_cinder_list_create_delet(self):
        ...
        volume = self.create_volume("hill_{0}".format(random.randint(1, 10000)), 1)
        ...
        for x in range(5):
            ...
            if volume["status"] == "available" or volume["status"] == "error":
                volume = self.delete_volume(self.tenant_id, volume["id"])
